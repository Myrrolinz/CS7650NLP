...
def parse_args():
    parser = argparse.ArgumentParser(description='Train LAMP')
    # general
...
def main_worker(rank, world_size, dist_url, final_output_dir, args):
    ...
    trainer = Trainer(cfg, model, rank, final_output_dir, writer_dict, awl)
    ...
    for epoch in range(begin_epoch, cfg.TRAIN.END_EPOCH):
        ...
        if (not cfg.DDP or (cfg.DDP and rank == 0)) and (epoch+1) % 5 == 0:
