...
class CID(nn.Module):
    def __init__(self, cfg, is_train):
        ...
        self.inst_loss_weight = cfg.LOSS.INST_LOSS_WEIGHT
        self.semantic_loss_weight = cfg.LOSS.JOINT_LOSS_WEIGHT
        self.pixel_loss_weight = cfg.LOSS.PIXEL_LOSS_WEIGHT
    ...
    def forward(self, batch_inputs):
        ...
        if self.training:
            ...
            if instances is None:
                single_heatmap_loss, contrastive_loss, ins_loss, semantic_loss, pixel_loss = torch.tensor(0), torch.tensor(0), torch.tensor(0), torch.tensor(0), torch.tensor(0)
            else:
            # limit max instances in training
                ...
                single_heatmap_loss, ins_loss, semantic_loss, pixel_loss = self.gfd(feats, instances)
            ...
            losses.update({'inst_loss': ins_loss * self.inst_loss_weight})
            losses.update({'semantic_loss': semantic_loss * self.semantic_loss_weight})
            losses.update({'pixel_loss': pixel_loss * self.pixel_loss_weight})
